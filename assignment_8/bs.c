...
void* matrixMul(void *arg)
{
	...
	for (int row = 0; row < 2; row++) 
	{
		...
		for (int i = 0; i < 2; i++)
		{
            ...
            //__m128 load_row_3 = _mm_load_ps(temp[2] + (row_1 * 4));                         //load row 3
			//__m128 load_row_4 = _mm_load_ps(temp[3] + (row_1 * 4));                         //load row 4
			__m128 add_row12 = _mm_add_ps(load_row_1, load_row_2);                          //add together
            //__m128 add_row34 = _mm_add_ps(load_row_3, load_row_4);                          //add together
            ...
            //resultant[column + 2][row] = ((float*)&add_row34)[0] + ((float*)&add_row34)[1]; //like classwork_7
			//resultant[column + 3][row] = ((float*)&add_row34)[2] + ((float*)&add_row34)[3]; // 
			if (column != 2)
			{
...
/***	
	...
	for (int row = 0; row < 4; row++) 
	{
		...
		for (int i = 0; i < 4; i++)
		{
			__m256 load_row_1 = _mm256_loadu_ps(temp[0] + (row_1 * 4));                         //load row 1
			__m256 load_row_2 = _mm256_loadu_ps(temp[1] + (row_1 * 4));                         //load row 2
			__m256 add_row12 = _mm256_hadd_ps(load_row_1, load_row_2);                          //add together
			resultant[column][row] = ((float*)&add_row12)[0] + ((float*)&add_row12)[1];         //like classwork_7
			resultant[column + 1][row] = ((float*)&add_row12)[2] + ((float*)&add_row12)[3];     // 
			if (column != 2)
			{
...
//repeat for the next two rows
	...
	for (int row = 2; row < 4; row++)
	{
		...
		for (int i = 2; i < 4; i++)
		{
			__m128 load_row_3 = _mm_load_ps(temp[2] + (row_2 * 4));                         //load row 3
			__m128 load_row_4 = _mm_load_ps(temp[3] + (row_2 * 4));                         //load row 4
